...
## Update 1.0
    - [x] implement upload pictures for portfolio. since we have limited storage, i want to use discord's image hosting service to store the pictures. so make a logic to add a new portfolio entry with : portfolio name, description, category, tags, year, area, status, image_url (give it a add more picture button), client, materials, price (optional), location, challenges, solutions, timeline, design, team members, and insights.
    - [x] refine the dashboard UI & UX. since this is a admin dashboard, make it more modern and user-friendly.
    - [x] setup a webhook to notify me if someone fill the contact form or submit a review.
    - [x] make sure the dashboard works properly.
...
## Update 1.4 New Feature and UI/UX Improvements and bug fixes
    - [x] make a review section on the home page. that will show latest review from client. make it dynamic. use the review data from the dashboard.
    - [x] make a new feature : task management for studio staff. studio manager can assign task to staff. staff can view their task and update the status. manager can view all task and update the status. and also can generate reports.
    - [x] make a new feature : timeline management for studio staff. studio manager can assign task to staff. staff can view their task and update the status. manager can view all task and update the status. and also can generate reports. 
    - [x] fix bugs : on the portfolio section, in the wide (luas) doesn't have m^2 text. 
    - [x] fix bugs : on the portfolio section, the name of the "pelaksana" doesn't show up.
    - [x] make slideshow for the portfolio.
    - [x] fix bugs : images on the portfolio is sometimes not showing up. 

## Update 1.5 invitation and login auth
    - [x] since we use supabase for database and auth. i want to invite staff to join the website admin page via supabase auth --> invite a new user. so we need to make a sign up page, login page, and forgot password page. And also we need to make a role management for staff and manager. and then make a system to invite staff to join the website admin page via supabase auth.
